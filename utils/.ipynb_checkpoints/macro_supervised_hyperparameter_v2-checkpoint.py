...
class MacroSupervised():
    def __init__(self, MacroDataset, MODEL, NUM_EPOCHS, NUM_WORKERS, CUSTOM_PARAMS, INFERENCE = None, MODEL_PATH=None, SAVE_MODEL=False, SAVE_OPT=False, SAVE_CONFIG=False):
        '''
    ...
    def _split_dataset(self):
        ...
        self.test_set = self._dataset.dgl_data['test']

        if self._inference is not None:
            self.inference_set = self._inference.dgl_data['train']
    ...
    def _run_a_train_epoch(self, epoch, model, data_loader, optimizer):
        ...
        '''
        epoch_log_idx = len(self._log)
        self._log.append([None] * 5)
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            self._log.append(['train', epoch + 1, batch_id + 1, print_val, batch_end_time - batch_start_time, ''])

        epoch_end_time = datetime.now()
        self._log[epoch_log_idx] = ['train', epoch + 1, '', '', epoch_end_time - epoch_start_time, '']
        
    def _run_an_eval_epoch(self, model, data_loader, type = '', epoch = None, log = False):
        ...
        with torch.no_grad():
            for batch_id, batch_data in enumerate(data_loader):
                ...
                if log == True:
                    diversity = False
                    if len(set(np.around(torch.sigmoid(logits)).flatten().tolist())) != 1:
                        diversity = True
                    self._log.append([type, epoch + 1, batch_id + 1, loss.item(), batch_end_time - batch_start_time, diversity])

        val_end_time = datetime.now()

        if log == True:
            self._log[val_log_idx] = [type, epoch + 1, "", "", val_end_time - val_start_time, '']
    ...
    def main(self):
        ...
        if self._inference is not None:
            inference_loader = DataLoader(dataset=self.inference_set, batch_size=self._exp_config['batch_size'],
                                 collate_fn=self._collate_molgraphs, num_workers=self._num_workers)
        ...
        for epoch in range(self._num_epochs):
            ...
            val_score = self._run_an_eval_epoch(model, val_loader, type = 'val', epoch = epoch, log = True)[0]
            
            if self._inference is not None:
                self._run_an_eval_epoch(model, inference_loader, type = 'inference', epoch = epoch, log = True)
        ...
        self._log = pd.DataFrame(self._log,columns=['type', 'epoch', 'batch', 'loss', 'elapsed_time', 'diversity'])
        self._log.to_csv(self._model_path + '/train_log.txt', index=None, sep=',', mode='w')
        ...
        self._best_val = self._run_an_eval_epoch(model, val_loader)
        self._best_val_score = self._best_val[0]
        self._val_plotvals = self._best_val[1]

        self._best_test = self._run_an_eval_epoch(model, test_loader)
        self._best_test_score = self._best_test[0]
        self._test_plotvals = self._best_test[1]
        ...
        return -self._best_val_score['precision']
        
    def rocauc_plot(self, plottype):
        ...
        if plottype == 'val':
            ...
            mean_fpr = self._test_plotvals[0]
            mean_tpr = self._test_plotvals[1]
        ...
        plt.tight_layout()
        plt.savefig(self._model_path + '/' + plottype + '_ROC_AUC.png')
        plt.show()
